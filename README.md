...
- Runs a grid search over imbalance lookback and EMA length while keeping a fixed take-profit of **0.34% price move** (≈3.4% at 10x leverage) to keep the live loop aligned with the optimizer.
...
- `imbalance_range`, `ema_range`: search spaces for the optimizer; **take-profit is fixed** via `SCALP_TP_PCT` to avoid overfitting.
